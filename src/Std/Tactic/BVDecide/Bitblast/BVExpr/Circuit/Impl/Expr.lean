...
where
  ...
  go {w : Nat} (aig : AIG BVBit) (expr : BVExpr w) (cache : Cache aig) : Return aig w :=
    ...
    | .un op expr =>
      ...
      | .sqrt =>
        let res := bitblast.blastSqrt eaig evec
        have := by
          apply AIG.LawfulVecOperator.le_size_of_le_aig_size (f := bitblast.blastSqrt)
          omega
        ⟨⟨res, this⟩, cache.cast (AIG.LawfulVecOperator.le_size (f := bitblast.blastSqrt) ..)⟩
    | .append lhs rhs h =>
...
theorem go_decl_eq (aig : AIG BVBit) (expr : BVExpr w) (cache : Cache aig) :
  ...
  next op expr =>
    match op with
    | .not | .rotateLeft .. | .rotateRight .. | .arithShiftRightConst .. | .reverse | .clz | .cpop | .sqrt =>
      rw [AIG.LawfulVecOperator.decl_eq]
      rw [goCache_decl_eq]
