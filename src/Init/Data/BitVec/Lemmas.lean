...
@[simp, grind =] theorem toFin_setWidth' {m n : Nat} (p : m ≤ n) (x : BitVec m) :
  ...
  ext
  rw [setWidth'_eq, toFin_setWidth, Fin.val_ofNat, Fin.castLE, val_toFin,
...
theorem List.exists_concat_of_length_eq_add_one : l.length = n + 1 → ∃ h t, l = t ++ [h] := by sorry


theorem concat_recursive_addition_list (newEl : Nat) (l : List Nat) :
  recursive_addition_list (l ++ [newEl]) = newEl + recursive_addition_list l := by sorry
...
def rec_add_eq_rec_add_iff (a b : List Nat) (n : Nat)
    ...
    (hn : n = a.length)
    (ha : ∀ i (hi : i < a.length) (hi' : 2 * i < b.length),
    ...
    · case _ n ihn =>
      ...
      rw [this]
      rw [ha (i := n) (by omega) (by omega)]
      split
      · case _ ht =>
        have : 2 ≤ b.length := by omega
        ...
        conv =>
          ...
          rw [concat_recursive_addition_list]
        have : b.length = tailb.length + 2 := by
          subst htailb; simp
        have : a.length = taila.length + 1 := by
          subst htaila; simp
        ...
        · omega
        · intros j hj hj'
          specialize ha j (by omega) (by omega)
