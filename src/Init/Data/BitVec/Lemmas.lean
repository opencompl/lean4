...
@[simp, grind =] theorem toInt_setWidth'_of_lt {m n : Nat} (p : m < n) {x : BitVec m} :
...
@[simp, grind =] theorem toInt_extractLsb {hi lo : Nat} {x : BitVec n} :
  (extractLsb hi lo x).toInt = ((x.toNat >>> lo) : Int).bmod (2 ^ (hi - lo + 1)) := by
  simp [extractLsb]

@[simp, grind =] theorem toFin_extractLsb' {s m : Nat} {x : BitVec n} :
    (extractLsb' s m x).toFin = Fin.ofNat (2 ^ m) (x.toNat >>> s) := by
  simp [extractLsb']

@[simp, grind =] theorem toFin_extractLsb {hi lo : Nat} {x : BitVec n} :
...
let x' = x.extractLsb' 7 5  =   _ _ 9 8 7
                                ...
                                ❌ start + len - i ≤ w
                                     7 + 5 - 0 ≤ w
-/
@[simp, grind =] theorem getMsbD_extractLsb' {start len : Nat} {x : BitVec w} {i : Nat} :
...
@[simp, grind =] theorem getMsbD_extractLsb {hi lo : Nat} {x : BitVec w} {i : Nat} :
...
@[simp, grind =] theorem toInt_allOnes : (allOnes w).toInt = if 0 < w then -1 else 0 := by
  norm_cast
  by_cases h : w = 0
  · subst h
    simp
...
@[simp, grind =] theorem toFin_allOnes : (allOnes w).toFin = Fin.ofNat (2^w) (2^w - 1) := by
  ext
  simp

@[simp, grind =] theorem getLsbD_allOnes : (allOnes v).getLsbD i = decide (i < v) := by
  simp [allOnes]
...
@[simp, grind =] theorem getElem_allOnes (i : Nat) (h : i < v) : (allOnes v)[i] = true := by
...
@[simp, grind =] theorem toFin_or (x y : BitVec v) :
    BitVec.toFin (x ||| y) = BitVec.toFin x ||| BitVec.toFin y := by
  apply Fin.eq_of_val_eq
  exact (Nat.mod_eq_of_lt <| Nat.or_lt_two_pow x.isLt y.isLt).symm

@[simp, grind =] theorem getLsbD_or {x y : BitVec v} : (x ||| y).getLsbD i = (x.getLsbD i || y.getLsbD i) := by
...
@[simp, grind =] theorem getElem_or {x y : BitVec w} {i : Nat} (h : i < w) : (x ||| y)[i] = (x[i] || y[i]) := by
...
@[simp, grind =] theorem ofNat_or {x y : Nat} : BitVec.ofNat w (x ||| y) = BitVec.ofNat w x ||| BitVec.ofNat w y :=
...
@[simp, grind =] theorem toFin_and (x y : BitVec v) :
    BitVec.toFin (x &&& y) = BitVec.toFin x &&& BitVec.toFin y := by
  apply Fin.eq_of_val_eq
  exact (Nat.mod_eq_of_lt <| Nat.and_lt_two_pow _ y.isLt).symm

@[simp, grind =] theorem getLsbD_and {x y : BitVec v} : (x &&& y).getLsbD i = (x.getLsbD i && y.getLsbD i) := by
  rw [← testBit_toNat, getLsbD, getLsbD]
  simp
...
@[simp, grind =] theorem getElem_and {x y : BitVec w} {i : Nat} (h : i < w) : (x &&& y)[i] = (x[i] && y[i]) := by
  simp [getElem_eq_testBit_toNat]
...
@[simp, grind =] theorem toFin_xor (x y : BitVec v) :
    BitVec.toFin (x ^^^ y) = BitVec.toFin x ^^^ BitVec.toFin y := by
  apply Fin.eq_of_val_eq
  exact (Nat.mod_eq_of_lt <| Nat.xor_lt_two_pow x.isLt y.isLt).symm

@[simp, grind =] theorem getLsbD_xor {x y : BitVec v} :
...
@[simp, grind =] theorem getElem_xor {x y : BitVec w} {i : Nat} (h : i < w) : (x ^^^ y)[i] = (x[i] ^^ y[i]) := by
  simp [getElem_eq_testBit_toNat]
...
@[simp, grind =] theorem ofNat_xor {x y : Nat} : BitVec.ofNat w (x ^^^ y) = BitVec.ofNat w x ^^^ BitVec.ofNat w y :=
...
@[simp, grind =] theorem toFin_not (x : BitVec w) :
    (~~~x).toFin = x.toFin.rev := by
  apply Fin.val_inj.mp
  simp only [val_toFin, toNat_not, Fin.val_rev]
  omega

@[simp, grind =] theorem getLsbD_not {x : BitVec v} : (~~~x).getLsbD i = (decide (i < v) && ! x.getLsbD i) := by
...
@[simp, grind =] theorem getElem_not {x : BitVec w} {i : Nat} (h : i < w) : (~~~x)[i] = !x[i] := by
...
@[simp, grind =] theorem getLsbD_shiftLeft (x : BitVec m) (n) :
...
@[simp, grind =] theorem getElem_shiftLeft {x : BitVec m} {n : Nat} (h : i < m) :
...
@[simp, grind =] theorem getMsbD_shiftLeftZeroExtend (x : BitVec m) (n : Nat) :
...
@[simp, grind =] theorem signExtend_and {x y : BitVec w} :
...
@[simp, grind =] theorem signExtend_not {x : BitVec w} (h : 0 < w) :
...
@[simp, grind =] theorem getMsbD_append {x : BitVec n} {y : BitVec m} :
...
@[simp, grind =] theorem toFin_append {x : BitVec m} {y : BitVec n} :
    (x ++ y).toFin =
...
@[simp, grind =] theorem toInt_concat (x : BitVec w) (b : Bool) :
...
@[simp, grind =] theorem toFin_concat (x : BitVec w) (b : Bool) :
...
@[simp, grind =] theorem fill_toNat {w : Nat} {v : Bool} :
    (fill w v).toNat = if v = true then 2^w - 1 else 0 := by
  by_cases h : v <;> simp [h]

@[simp, grind =] theorem fill_toInt {w : Nat} {v : Bool} :
    (fill w v).toInt = if v = true && 0 < w then -1 else 0 := by
  by_cases h : v <;> simp [h]

@[simp, grind =] theorem fill_toFin {w : Nat} {v : Bool} :
...
@[simp, grind =] theorem getMsbD_ofBoolListBE : (ofBoolListBE bs).getMsbD i = bs.getD i false := by
  induction bs generalizing i <;> cases i <;> simp_all [ofBoolListBE]

@[simp, grind =] theorem getLsbD_ofBoolListBE :
...
@[simp]
theorem flatRecAdd_zero (x : BitVec (l * w)) (acc : BitVec w):
  x.flatRecAdd 0 acc = acc := rfl

@[simp]
theorem flatRecAdd_succ (x : BitVec (l * w)) (n : Nat) (acc : BitVec w) :
  x.flatRecAdd (n + 1) acc = x.flatRecAdd n (acc + extractLsb' (n * w) w x) := rfl

theorem flatRecAdd_zero_add (x : BitVec (l * w)) (n : Nat) (acc : BitVec w) :
    (x.flatRecAdd n 0#w) + acc = x.flatRecAdd n acc := by
  induction n generalizing acc
  · simp
  · case _ n ihn =>
    have ih1 := ihn (acc := extractLsb' (n * w) w x)
    have ih2 := ihn (acc := acc + extractLsb' (n * w) w x)
    simp only [flatRecAdd_succ, BitVec.zero_add, ← ih1, BitVec.add_assoc, ← ih2,
      BitVec.add_right_inj]
    rw [BitVec.add_comm (x := acc)]

theorem flatRecAdd_add (x : BitVec (l * w)) (n : Nat) (acc acc' : BitVec w) :
    x.flatRecAdd n (acc + acc') = (x.flatRecAdd n acc) + acc' := by
  simp [← flatRecAdd_zero_add (acc := acc + acc'), ← flatRecAdd_zero_add (acc := acc), BitVec.add_assoc]

theorem flatRecAdd_extractLsb' (a : BitVec (length * w)) (h : r ≤ length):
    a.flatRecAdd r 0#w =
    (extractLsb' 0 (r * w) a).flatRecAdd r 0#w:= by
  induction r generalizing a length
  · simp [flatRecAdd]
  · case _ diff ihdiff =>
    simp only [flatRecAdd_succ, BitVec.zero_add]
    rw [extractLsb'_extractLsb'_of_le (by simp [Nat.add_mul]), ← flatRecAdd_zero_add,
      ← flatRecAdd_zero_add (x := extractLsb' 0 ((diff + 1) * w) a),
...
theorem recursive_addition_eq_of_le' (a : BitVec (length * w)) (h : r ≤ length) (hk : r ≤ k):
    a.flatRecAdd r 0#w =
    (extractLsb' 0 (k * w) a).flatRecAdd r 0#w:= by
  induction r generalizing a length k
  · simp [flatRecAdd]
  · case _ diff ihdiff =>
    simp only [flatRecAdd_succ, BitVec.zero_add]
    rw [extractLsb'_extractLsb'_of_le
        (by simp only [show diff * w + w = (diff + 1) * w by simp [Nat.add_mul]]; exact Nat.mul_le_mul_right w hk), ← flatRecAdd_zero_add, ← flatRecAdd_zero_add (x := extractLsb' 0 (k * w) a),
...
theorem cast_recursive_addition_eq_of_le' {a_length' w : Nat} (a : BitVec ((a_length'.succ + 1) * w)):
    let hcast : w + (a_length'.succ + 1 - 1) * w = (a_length'.succ + 1) * w := by
      simp [Nat.add_mul]
      omega
    (BitVec.cast hcast (extractLsb' ((a_length' + 1) * w) w a ++ extractLsb' 0 ((a_length' + 1) * w) a)).flatRecAdd
    a_length' 0#w =
    (extractLsb' 0 (a_length' * w) a).flatRecAdd a_length' 0#w := by
  have : extractLsb' ((a_length' + 1) * w) w a ++ extractLsb' 0 ((a_length' + 1) * w) a = a.cast (by simp [Nat.add_mul]; omega) := by
    ext k hk
    simp only [Nat.succ_eq_add_one, getElem_append, getElem_extractLsb', Nat.zero_add, dite_eq_ite,
      getElem_cast]
    split
    · rw [← getLsbD_eq_getElem]
    · simp [show (a_length' + 1) * w + (k - (a_length' + 1) * w) = k by omega, ← getLsbD_eq_getElem]
  simp only [this, Nat.succ_eq_add_one, Nat.add_one_sub_one, cast_cast, cast_eq]
...
theorem flatRecAdd_append_extractLsb' {a : BitVec (a_length * w)} (ha : 0 < a_length) :
    let hc : w + (a_length - 1) * w = a_length * w := by
      simp [Nat.sub_mul]; rw [← Nat.add_sub_assoc (by exact Nat.le_mul_of_pos_left w ha)]; omega
    ((a.extractLsb' ((a_length - 1) * w) w ++ a.extractLsb' 0 ((a_length - 1) * w)).cast hc).flatRecAdd a_length 0#w
    =
      a.extractLsb' ((a_length - 1) * w) w + (a.extractLsb' 0 ((a_length - 1) * w)).flatRecAdd (a_length - 1) 0#w
     := by
  ...
  · omega
  · case _ a_length' iha' =>
    simp only [Nat.add_one_sub_one, flatRecAdd_succ, BitVec.zero_add]
    conv =>
      rhs
      unfold flatRecAdd
    split
    · ext k hk
      simp [newEl, getLsbD_append, flatRecAdd]
      omega
    · case _ a_length' =>
      simp only [flatRecAdd_succ]
      ...
      congr
      simp only [Nat.succ_eq_add_one, Nat.add_one_sub_one, BitVec.zero_add, newEl]
      rw [← flatRecAdd_zero_add, cast_recursive_addition_eq_of_le',
          ← recursive_addition_eq_of_le' (h := by omega),
          ← flatRecAdd_zero_add (x := extractLsb' 0 ((a_length' + 1) * w) a),
          ← recursive_addition_eq_of_le' (h := by omega) (hk := by omega)]
      simp only [Nat.succ_eq_add_one, ← BitVec.add_assoc, BitVec.add_left_inj]
      rw [BitVec.add_comm]
      omega

theorem flatRecAdd_eq_of
    (a : BitVec (a_length * w))
    ...
    (n : Nat)
    (hn : n = a_length) :
      a.flatRecAdd a_length 0#w = b.flatRecAdd b_length 0#w := by
  ...
  · case _ n' ihn =>
    ...
    rw [extractLsb'_append_extractLsb'_eq_of_lt (a := a) (by omega),
      flatRecAdd_append_extractLsb' (by omega), hadd (i := a_length - 1) (by omega)]
    ...
    · conv =>
        ...
        rw [extractLsb'_append_extractLsb'_eq_of_lt (a := b) (by omega),
            flatRecAdd_append_extractLsb' (by omega),
            extractLsb'_append_extractLsb'_eq_of_lt (a := extractLsb' 0 ((b_length - 1) * w) b) (by omega), flatRecAdd_append_extractLsb' (by omega)]
