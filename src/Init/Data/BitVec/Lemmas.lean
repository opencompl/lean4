...
@[simp]
theorem msb_neg (x : BitVec w) : (- x).msb = (decide ((w != 0) && ((x = intMin w) || !x.msb))) := by
  rw [msb_eq_decide]
  rcases w with rfl | w 
...
theorem toInt_abs (x : BitVec w) : x.abs.toInt = if x = intMin w then (intMin w).toInt else x.toInt.abs := by
  · rw [abs_def]
    rw [toInt_eq_msb_cond]
    split 
    case isTrue h =>
