...
/-- Simplification procedure for `popCount` (population count) on `BitVec`. -/
builtin_dsimproc [simp, seval] reducepopCount (popCount _) := reduceUnary ``BitVec.popCount 2 BitVec.popCount
